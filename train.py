...
def load_labels(csv_path):
    ...
    with open(csv_path) as f:
        r = csv.reader(f)
        next(r)  # saltar cabecera
        for path, label in r:
            y.append(int(label))
    return np.array(y)

def main(emb_path='data/embeddings.npy', labels_csv='data/labels.csv',
         model_out='models/model.joblib', scaler_out='models/scaler.joblib',
    ...
    clf = LogisticRegression(
        max_iter=200,
        multi_class='auto',   # importante para 3 clases
        solver='lbfgs'        # compatible con multiclase
    )
    clf.fit(X_train_s, y_train)

    y_pred = clf.predict(X_val_s)
    y_proba = clf.predict_proba(X_val_s)  # ahora usamos todas las clases

    # ROC-AUC multiclase (macro, one-vs-rest)
