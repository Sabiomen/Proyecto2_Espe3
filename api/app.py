...
# THRESHOLD ya no se usa, pero lo dejamos por compatibilidad
...
# Mapa entre id numérico y nombre de clase
# según cómo entrenaste: 0=user_2, 1=user_1, 2=unknown
CLASS_NAMES = {
    0: 'user_2',
    1: 'user_1',
    2: 'unknown'
...
def process_image(file):
    ...
    proba = clf.predict_proba(emb_s)[0]          # vector de probs por clase
    pred_id = int(clf.predict(emb_s)[0])         # etiqueta numérica 0/1/2
    pred_class = CLASS_NAMES.get(pred_id, 'unknown')

    # Probabilidad asociada a la clase predicha
    # proba está alineado con clf.classes_
    class_probs = {
        CLASS_NAMES[int(c)]: float(proba[i])
        for i, c in enumerate(clf.classes_)
    }
    score = float(class_probs[pred_class])
...
def verify():
    ...
    pred_id = int(clf.predict(emb_s)[0])
    pred_class = CLASS_NAMES.get(pred_id, 'unknown')
